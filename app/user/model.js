...
userSchema.path("email").validate(
  function (value) {
    ...
    // (2) test email, hasilnya adalah `true` atau `false` // jika ternyata `true` maka validasi berhasil
...
userSchema.path("email").validate(
  async function (value) {
    try {
      ...
      // jika `false` maka validasi gagal // jika `true` maka validasi berhasil return !count;
    } catch (err) {
      throw err;
    }
  },
